...
@section('tab-content')
<script> //to check if it is alumni or admin
...
<link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet"> <!-- improve visual using Tailwind -->
    <div class="max-w-2xl mx-auto p-4" x-data="postApp({{ auth()->id() }})" x-init="fetchPosts()">
        <h2 class="text-xl font-bold mb-4">Create a Business Listing</h2>
    ...
    <script>
        ...
        return {
            ...
            saveEditedPost(post) {
                ...
                const editedContent = post.editedContent ? post.editedContent.trim() : '';
            ...
            saveEditedComment(comment, postId) {
                ...
                fetch(`/businesslistingcomments/${comment.id}`, {
                    method: 'PUT',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
            ...
            fetchPosts() {
                ...
                fetchComments(postId, post) {
                    fetch(`/businesslistingposts/${postId}/comments`)
                        .then(res => res.json())
                        .then(comments => {
                ...
                addComment(postId, content) {
                    ...
                    fetch(`/businesslistingposts/${postId}/comment`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                            'Content-Type': 'application/json'
                ...
                deleteComment(commentId, postId) {
                    ...
                    fetch(`/businesslistingcomments/${commentId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                        }
                ...
                likePost(postId) {
                    fetch(`/businesslistingposts/${postId}/like`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                        }
